...
namespace GraphicLibrary.Cameras
{
    ...
    {
        public StaticCamera(Vector3 position, Vector3 target, CameraSettings settings) : base(position, target, settings)
        {
        }

        public StaticCamera(Vector3 position, Vector3 target, CameraSettings settings, ModelUnit unit) : base(position, target, settings, unit)
        {
        }

        public StaticCamera(Vector3 position, Vector3 target, CameraSettings settings, IEnumerable<ModelUnit> units) : base(position, target, settings, units)
        {
        }
